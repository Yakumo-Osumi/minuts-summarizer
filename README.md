...
転職ポートフォリオとして作成しました。業務効率化 × AI をテーマに、Phase 1（AWSなし構成）から段階的にAWS構成へ移行していく設計を採用しています。
...
- **Phase 1（現在）**: AWSなし構成（React + Express）で動くものを作る
- **Phase 2**: バックエンドをAWS Lambda + API Gatewayに移行
- **Phase 3**: フロントエンドをS3 + CloudFrontでホスティング
