...
提案前に、ローカルとリモート両方で同名ブランチの有無を確認する。ローカルのみのチェック（`git branch --list`）ではリモートにのみ存在するブランチを見逃すため、必ず両方を見る：

```bash
# ローカル
git branch --list <ブランチ名>

# リモート（ローカルにfetch済みでなくても直接確認できる）
git ls-remote --heads origin <ブランチ名>
```

いずれかに既に存在する場合は「作成」ではなく「切り替え」を提案する旨をユーザーに伝える。

## 作成の実行

ユーザーが明示的に承認した後にのみ、確認結果に応じて以下を実行する：

```bash
# ローカル・リモートいずれにも存在しない場合
git checkout -b <ブランチ名>

# ローカルに存在する場合
git checkout <ブランチ名>

# リモートにのみ存在する場合（ローカルに追跡ブランチを作成して切り替え）
