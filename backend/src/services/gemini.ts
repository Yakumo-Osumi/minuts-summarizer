...
2.「次回までに」「来週まで」等の相対的な期限も、期限ありとして扱う（変換要否・出力形式の判断基準はエッジケース4・5を参照）。
...
## 迷ったときの判断基準（エッジケース）
1. 担当者が曖昧なときは、assigneeを"不明"とする（タスクに含める）
2. 期限が"〜月初旬"、"〜月中旬"、"〜月下旬"などの曖昧なときは"(仮)"を先頭に付け具体的な日付をつける（例: "4月上旬までに"→"2026年4月8日"）
3. 行動が曖昧なとき、"検討する"などは抽象度を下げた表現に言い換える（例: "体制について見直す"→"新体制の案を資料にまとめる"）
4. 基準が分かる相対期限（来週・今月末など、会議日から算出可能）は議事録本文に計算の基準となる日付（会議開催日、または次回開催日など）が明記されている場合、その日付から具体的な日付を計算し、"まで"などの助詞を付けずに出力すること（例：入力が"来週まで"でも → ○ "2026年10月14日" / ✗ "2026年10月14日まで"）。
   基準となる日付は、当該の文の近くだけでなく、議事録全体（連絡事項・その他 等、離れた箇所も含む）から探すこと。
   例：本文の別の箇所（例："その他"）に「次回定例は10月14日」とあれば、その文と離れていても「次回まで」は「2026年10月14日」に変換する。
   基準となる日付が本文のどこにも見当たらない場合のみ、相対表現のまま "○○まで" という形式で出力すること（例：次回まで、来週まで）。"次回" のように「まで」を省略した形では出力しないこと。
5. 日付を出力したものは助詞を付けない（例：○ "2026年10月14日" / ✗ "2026年10月14日まで"）。

## 各フィールドの定義
- assignee: タスクを実行する人達の名前。承認者や確認者は含まない。敬称を付けない。誰が行うか決まっていない場合は"不明"とする。
- content: 何をするかという情報を持つフィールド。
- deadline: タスクの期限を持つフィールド。日付を出力したものは助詞を付けない。`;
