...
| フェーズ | 内容                                    | 状態                                   |
| -------- | --------------------------------------- | -------------------------------------- |
| Phase 1  | AWSなし構成で動くものを作る             | ✅ 完了                                |
| Phase 2  | バックエンドをAWS Lambdaに移行          | 意図的に保留中（優先度：品質改善を優先） |
| Phase 3  | S3 + CloudFrontでフロントをホスティング | 意図的に保留中（優先度：品質改善を優先） |
...
Phase 1 完了後、Phase 2（AWS移行）には着手せず、以下の品質改善を優先して進める。

- タスク一覧のコピー/エクスポート機能（Markdown / CSV）
- タスク抽出の再現率評価（正解ラベル付け → 抽出結果と突き合わせて再現率を算出 → 改善）

### Step 1：タスク一覧のコピー/エクスポート機能

- [ ] Markdown形式でのエクスポート機能を実装
- [ ] CSV形式でのエクスポート機能を実装
- [ ] エクスポートボタンをUIに追加

### Step 2：タスク抽出の再現率評価

- [ ] 正解ラベル付きの評価用データセットを作成
- [ ] 抽出結果と正解ラベルを突き合わせて再現率を算出するスクリプトを作成
- [ ] 評価結果をもとにプロンプトを改善
